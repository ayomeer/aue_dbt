-- View: prod_gl_arten.view_artvorkommen_kategorisiert

-- DROP VIEW prod_gl_arten.view_artvorkommen_kategorisiert;

CREATE OR REPLACE VIEW prod_gl_arten.view_artvorkommen_kategorisiert AS 
 SELECT row_number() OVER ()::integer AS vid,
   ...
   FROM prod_gl_arten.cat_art c
     LEFT JOIN prod_gl_arten.view_artvorkommen_pt_artnamen f ON c.id_art = f.id_from_cat_arten
  ORDER BY c.artengruppe, c.bez_art_latein;

ALTER TABLE prod_gl_arten.view_artvorkommen_kategorisiert
  OWNER TO prod_gl_arten_write;
GRANT ALL ON TABLE prod_gl_arten.view_artvorkommen_kategorisiert TO prod_gl_arten_write;
GRANT SELECT, REFERENCES ON TABLE prod_gl_arten.view_artvorkommen_kategorisiert TO prod_gl_arten_read;
GRANT SELECT ON TABLE prod_gl_arten.view_artvorkommen_kategorisiert TO prod_gl_arten_foerster;
COMMENT ON VIEW prod_gl_arten.view_artvorkommen_kategorisiert
